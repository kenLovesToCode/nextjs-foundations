...
export default async function ErrorDemoPage(props: {
  ...
  return (
    <main className="mx-auto max-w-2xl p-8">
      ...
      <Suspense fallback={<ErrorDemoControlsSkeleton />}>
        <ErrorDemoControls searchParams={props.searchParams} />
      </Suspense>
    </main>
  );
}

async function ErrorDemoControls(props: {
  searchParams: Promise<{ throw?: string }>;
}) {
...
function ErrorDemoControlsSkeleton() {
  return (
    <div className="space-y-4">
      <div className="h-36 animate-pulse rounded border bg-gray-100" />
      <div className="h-36 animate-pulse rounded border bg-gray-100" />
    </div>
  );
}
